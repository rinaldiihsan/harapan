...
import ProgramUnggulanForm from './ProgramUnggulanForm';
...
export default function ProgramUnggulanClient() {
  const [list, setList] = useState<ProgramUnggulan[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isDeleting, setIsDeleting] = useState(false);
  const [openForm, setOpenForm] = useState(false);
  const [openDelete, setOpenDelete] = useState(false);
  const [selected, setSelected] = useState<ProgramUnggulan | null>(null);

  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await fetch('/api/program-unggulan');
      const data = await res.json();
      setList(data.data ?? []);
    } catch {
      toast.error('Gagal memuat data program unggulan');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleDeleteConfirm = (item: ProgramUnggulan) => {
    setSelected(item);
    setOpenDelete(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/program-unggulan/${selected.id}`, {
        method: 'DELETE',
      ...
      if (!res.ok) {
        toast.error(result.message ?? 'Gagal menghapus program unggulan');
        return;
      }
  ...
  return (
    <div className="pt-16">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-black">Program Unggulan</h1>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-black">Nama Program</TableHead>
      ...
      <Dialog open={openForm} onOpenChange={setOpenForm}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-black">{selected ? 'Edit Program Unggulan' : 'Tambah Program Unggulan'}</DialogTitle>
          </DialogHeader>
          <ProgramUnggulanForm
            data={selected}
            onSuccess={() => {
      ...
      <Dialog open={openDelete} onOpenChange={setOpenDelete}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-black">Hapus Program Unggulan</DialogTitle>
          </DialogHeader>
          <p className="text-black text-sm">
            Yakin ingin menghapus program <span className="font-medium">"{selected?.name}"</span>? Tindakan ini tidak dapat dibatalkan.
          </p>
